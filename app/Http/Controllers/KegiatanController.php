...
class KegiatanController extends Controller
{
    ...
    function penjualan()
    {
        $penjualan = DB::table('penjualan')
            ->join('detail_penjualan', 'detail_penjualan.detail_id', '=', 'penjualan.penjualan_id')
            ->select('penjualan.*', 'detail_penjualan.detail_id')
            ->get();

        return view('/penjualan', ['penjualan' => $penjualan])->with('pejualan', $penjualan);
    }
    ...
    public function tambah_penjualan()
    {
        ...
        ->join('produk', 'produk.produk_id', '=', 'detail_penjualan.produk_id') // Corrected alias
        ->select('detail_penjualan.*', 'produk.*')
        ->get();

        $penjualan = DB::table('penjualan')
        ->join('pelanggan', 'penjualan.pelanggan_id', '=', 'pelanggan.pelanggan_id')
        ->select('pelanggan.*', 'penjualan.*')
        ->get();

        return view('/tambah_penjualan', ['produk' => $produk,'pelanggan'=>$pelanggan,'detail_penjualan'=>$detail_penjualan,'penjualan'=>$penjualan]);
    }
